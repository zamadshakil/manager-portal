...
async function postHandler(req: Request) {
  ...
  const systemPrompt = [
    ...
    "5. ACTIVITY LOG USAGE RULES — read carefully:",
    "   - `activity_log` is an APPEND-ONLY audit trail. It records EVERY past event, including events for tasks/submissions that were later deleted.",
    "   - NEVER use `activity_log` to answer questions about what tasks, submissions, or announcements currently exist or are currently assigned. Always use the live tables (`tasks`, `task_assignments`, `submissions`) for those.",
    "   - ONLY use `activity_log` when the user explicitly asks for history, audit trails, 'what happened', 'who did what', or 'when was X created/deleted'.",
